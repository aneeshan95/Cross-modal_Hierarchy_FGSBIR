...
class Joint_embdedding(nn.Module):
    def __init__(self):
        ...
        self.linear_embed = nn.Sequential(nn.Linear(768+2048, 512), nn.ReLU(True),
            nn.Dropout(0.5), nn.Linear(512, 64))

    def forward(self, raster_sketch, vector_sketch, seq_len):
        vector_emb = self.vector_emb(vector_sketch, seq_len)
        raster_emb = self.raster_emb(raster_sketch)
...
class Vector_embedding(nn.Module):
    ...
    def forward(self, x, seq_len):
        ...
        src_key_pad_mask = torch.zeros((x.shape[1], x.shape[0]), dtype=torch.bool)
        ...
        x_hidden = self.linear_embed(x_hidden)


        return F.normalize(x_hidden)
...
class Vector_embedding2(nn.Module):
    def __init__(self):
        ...
        self.linear_embedding = nn.Linear(1024,64)
