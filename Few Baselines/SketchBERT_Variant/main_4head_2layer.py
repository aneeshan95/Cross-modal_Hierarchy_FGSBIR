...
from Net_Basic_V1_4h_2l import Net_Basic, Vector_embedding
import time
...
def main_train(opt):
    ...
    for epoch in range(opt.niter):
        for i, sanpled_batch in enumerate(dataloader_sketchy_train, 0):
            ...
            sketch_anchor_embedding = model_sketch(sanpled_batch['sketch_coord'].to(device), sanpled_batch['seq_len'].to(device))
            rgb_positive_embedding = model_image(sanpled_batch['positive_img'].to(device))
            rgb_negetive_embedding = model_image(sanpled_batch['negetive_img'].to(device))
            ...
            if (i + 0) % opt.save_iter == 0:
                ...
                if top1 > top1_buffer:
                    torch.save(model_sketch.state_dict(), 'model_Best_sketch4h2l.pth')
                    torch.save(model_image.state_dict(), 'model_Best_image4h2l.pth')
