...
class CreateDataset_Sketchy(data.Dataset):
    def __init__(self, opt, on_Fly=False):
        ...
        with open('./../Data/ChairV2_Select_Search_boxes_min10_topN16.pickle', 'rb') as handle:
        ...
        self.on_Fly = on_Fly
        # for i in self.pic_box_dict:        # in case no. of regions are an issue
        #     if len(self.pic_box_dict[i]) < 20:
        #         print(i)
        #         if '/train/'+i[:-4] in self.Skecth_Train_List:
        #             self.Skecth_Train_List.remove('/train/'+i[:-4])
        #             print('tr: ', i)
        #         elif '/test/' + i[:-4] in self.Skecth_Train_List:
        #                 self.Skecth_Train_List.remove('/train/' + i[:-4])
        #                 print('te: ', i)
        # print('Done')
    ...
    def __getitem__(self, item):
        ...
        if self.opt.mode == 'Train':
            ...
            positive_boxes = self.pic_box_dict[positive_sample+'.png'][:14]
            negetive_img = self.transform(negetive_img)
            negetive_boxes = self.pic_box_dict[negetive_sample+'.png'][:14]
...
if __name__ == "__main__":
    ...
    opt.roor_dir = '/home/media/On_the_Fly/Code_ALL/Dataset_Three/ChairV2/'
    opt.mode = 'Train'
    opt.Train = True
    opt.shuffle = True
    opt.nThreads = 1
